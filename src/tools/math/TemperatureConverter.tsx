        ...
        <div className="flex flex-col h-full gap-6">
            <div className="glass-panel p-8 flex flex-col items-center gap-12">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <h3 className="text-lg font-bold text-white/80">Temperature Converter</h3>
                </div>
                        ...
                        <div className="flex gap-1 p-1 bg-white/5 rounded-xl border border-white/10">
                            {(['C', 'F', 'K'] as Unit[]).map(u => (
                                <button
                                    ...
                                    className={`w-12 h-12 rounded-lg font-bold transition-all ${unit === u
...
const ResultCard: React.FC<{ label: string; value: number; unit: string; active: boolean }> = ({ label, value, unit, active }) => (
    <div className={`p-4 rounded-xl border transition-all ${active
            ? 'bg-indigo-500/10 border-indigo-500/50 scale-105 shadow-lg'
            : 'bg-white/5 border-white/10 opacity-70'
        }`}>
        <span className="text-[10px] uppercase tracking-widest text-white/40 font-bold block mb-1">{label}</span>
