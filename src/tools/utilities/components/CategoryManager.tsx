...
export const CategoryManager: React.FC<CategoryManagerProps> = ({ itemId, onClose }) => {
    ...
    return (
        ...
        >
            <div className="w-full max-w-2xl max-h-[80vh] glass-panel border border-[var(--color-glass-border)] rounded-xl shadow-2xl flex flex-col animate-in zoom-in-95 duration-200">
                {/* Header */}
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="flex gap-2">
                                <input
                                    type="text"
                                    className="flex-1 px-4 py-2 glass-input border border-[var(--color-glass-border)] rounded-lg 
                        ...
                        <div className="space-y-3">
                            ...
                            {availableCategories.length === 0 ? (
                                <div className="p-8 text-center bg-[var(--color-glass-input)] border border-[var(--color-glass-border)] rounded-lg">
                                        ...
                                        return (
                                            <div
                                                ...
                                                className={`p-3 border rounded-lg transition-all ${itemId
                                                        ? isSelected
                                                            ? 'bg-indigo-500/10 border-indigo-500 cursor-pointer hover:bg-indigo-500/20'
                                                            : 'bg-[var(--color-glass-input)] border-[var(--color-glass-border)] cursor-pointer hover:border-indigo-500/50'
                                                        : 'bg-[var(--color-glass-input)] border-[var(--color-glass-border)]'
                                                    }`}
                                                onClick={() => itemId && handleToggleItemCategory(category)}
                                            >
                                                <div className="flex items-center justify-between">
                                                    <div className="flex items-center gap-2 flex-1 min-w-0">
                                                        <Tag className={`w-4 h-4 flex-shrink-0 ${isSelected ? 'text-indigo-400' : 'text-foreground-muted'
                                                            }`} />
                                                        <span className={`text-sm font-medium truncate ${isSelected ? 'text-indigo-400' : 'text-foreground'
                                                            }`}>
                                                            {category}
                                                        </span>
                                                    </div>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {itemCategories.map((category) => (
                                        <div
                                            key={category}
                                            className="px-3 py-1.5 bg-indigo-500/10 border border-indigo-500 rounded-full 
                                                     flex items-center gap-2 text-sm text-indigo-400"
                                        >
