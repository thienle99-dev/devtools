  ...
  return (
    ...
    >
      <div className="h-full flex flex-col md:flex-row gap-6 p-4">
        <div className="flex-1 flex flex-col items-center justify-center p-8 text-center space-y-8 bg-gray-900/10 rounded-xl border border-gray-800/50">
            {!keyInfo ? (
                <div className="text-gray-500 animate-pulse flex flex-col items-center">
                    <div className="w-24 h-24 border-2 border-dashed border-gray-600 rounded-xl flex items-center justify-center mb-4">
                ...
                <div className="animate-in fade-in zoom-in duration-200">
                    <div className="text-8xl font-bold text-white mb-2 font-mono">
        ...
        <div className="flex-1 overflow-auto custom-scrollbar">
            {!keyInfo ? (
                <div className="flex items-center justify-center h-full text-gray-500 italic p-8 border border-dashed border-gray-800 rounded-xl">
                    Event details will appear here...
                </div>
            ) : (
                <div className="grid grid-cols-1 gap-4">
                    <Card className="p-4 space-y-4">
                        <h3 className="font-semibold text-lg text-white border-b border-gray-800 pb-2">Event Properties</h3>
                    ...
                    <Card className="p-4 space-y-4">
                        <h3 className="font-semibold text-lg text-white border-b border-gray-800 pb-2">Modifiers & State</h3>
                     ...
                     <Card className="p-4 space-y-4">
                        <h3 className="font-semibold text-lg text-white border-b border-gray-800 pb-2">Details</h3>
...
const InfoRow = ({ label, value, deprecated, type = 'text' }: { label: string, value: any, deprecated?: boolean, type?: 'text' | 'boolean' }) => (
    <div className="flex justify-between items-center py-2 border-b border-gray-800/50 last:border-0">
        <span className="text-gray-400 font-mono text-sm flex items-center">
            {label}
            {deprecated && <span className="ml-2 text-[10px] bg-red-900/50 text-red-300 px-1.5 py-0.5 rounded uppercase">Deprecated</span>}
        </span>
        {type === 'boolean' ? (
             <span className={`text-sm font-mono px-2 py-0.5 rounded ${value ? 'bg-green-900/40 text-green-400' : 'bg-gray-800 text-gray-500'}`}>
                {value.toString()}
            </span>
        ) : (
             <span className="text-white font-mono text-lg font-semibold bg-gray-900/50 px-2 rounded">
