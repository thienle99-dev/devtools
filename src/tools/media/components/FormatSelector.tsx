...
export const FormatSelector: React.FC<FormatSelectorProps> = ({
    ...
    const estimateSize = (q: string): string => {
        ...
        return formatBytes(sizeEst * 1024 * 1024);
    };
        ...
        <div className="space-y-4">
            ...
            <div className="bg-background-tertiary/50 p-1.5 rounded-2xl border border-border-glass flex gap-1">
                <button
                    ...
                    disabled={disabled}
                    className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold transition-all ${format === 'video'
                ...
                <button
                    ...
                    disabled={disabled}
                    className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold transition-all ${format === 'audio'
                ...
                <div className="p-4 border-b border-border-glass bg-background-secondary/30">
                        ...
                        <div className="flex gap-2">
                            {(format === 'video' ? ['mp4', 'mkv', 'webm'] : ['mp3', 'm4a', 'wav']).map(fmt => (
                                <button
                                    ...
                                    disabled={disabled}
                                    className={`px-3 py-1 rounded-lg text-[10px] font-bold uppercase transition-all border ${container === fmt
                                        ? format === 'video'
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <button
                            ...
                            className={`flex items-center gap-2 px-3 py-1 rounded-lg text-[10px] font-bold uppercase transition-all border ${embedSubs
                        ...
                        (videoInfo?.availableQualities || ['1080p', '720p', '480p']).map((q: string) => {
                            ...
                            return (
                                <div
                                    ...
                                    className={`w-full flex items-center justify-between p-3 rounded-xl border transition-all group cursor-pointer ${isRecommended
                                        ? 'border-green-500/30 bg-green-500/5'
                                        : 'border-transparent hover:bg-white/5 hover:border-white/5 bg-background-tertiary/20'
                                        }`}
                                >
                                    <div className="flex items-center gap-4">
                                        <div className={`p-2 rounded-lg ${isRecommended
                                        ...
                                        <div className="text-left">
                                            <div className={`text-sm font-bold flex items-center gap-2 ${isRecommended ? 'text-green-400' : 'text-foreground-primary group-hover:text-red-400'
                                                } transition-colors`}>
                                                {q}
                                                {isRecommended && (
                                    ...
                                    <div className="text-right flex items-center gap-3">
                                        ...
                                        <div className={`p-2 rounded-lg transition-all shadow-sm ${isRecommended
