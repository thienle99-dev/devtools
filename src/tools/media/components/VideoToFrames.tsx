...
export const VideoToFrames: React.FC = () => {
    ...
    const handleDragOver = (e: React.DragEvent, _: number) => {
        e.preventDefault();
                                                ...
                                                <div className={`grid gap-4 ${viewMode === 'grid' ? 'grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5' : 'grid-cols-1'}`}>
                                                    {frames.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE).map((frame, idx) => {
                                                        ...
                                                        return (
                                                            <div
                                                                key={frame.index}
                                                                className={`relative group rounded-xl overflow-hidden border transition-all duration-200 ${isSelected
                                                            ...
                                                            >
                                                                {/* Selection Overlay */}
                                                                <div
                                                                    className={`absolute inset-0 z-10 transition-colors cursor-pointer ${isSelected ? 'bg-indigo-500/10' : 'hover:bg-white/5'
                                                                        }`}
                                                                    onClick={(e) => {
