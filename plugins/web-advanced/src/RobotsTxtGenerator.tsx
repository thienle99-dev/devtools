...
const TOOL_ID = TOOL_IDS.ROBOTS_TXT_GENERATOR;

export const RobotsTxtGenerator: React.FC<BaseToolProps> = ({ tabId }) => {
  const effectiveId = tabId || TOOL_ID;
  const { data: toolData, setToolData, clearToolData, addToHistory } = useToolState(effectiveId);

  const data = toolData || {
    options: {
      rules: [{ userAgent: '*', allow: [], disallow: [] }] as Rule[],
      sitemap: ''
    }
  };

  const { rules, sitemap } = data.options;
  ...
  const addRule = () => {
    updateOptions({ rules: [...rules, { userAgent: '*', allow: [], disallow: [] }] });
  };

  const removeRule = (index: number) => {
    const newRules = [...rules];
    newRules.splice(index, 1);
    updateOptions({ rules: newRules });
  };
  ...
  const handleReset = () => clearToolData(effectiveId);
        ...
        <div className="flex-1 flex flex-col gap-4 overflow-y-auto custom-scrollbar">
          <Card className="p-4 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                value={sitemap}
                onChange={(e) => updateOptions({ sitemap: e.target.value })}
