...
type ReactFlowModule = typeof import('reactflow');
let reactFlowModulePromise: Promise<ReactFlowModule> | null = null;

function loadReactFlowModule() {
    if (!reactFlowModulePromise) {
        reactFlowModulePromise = import('reactflow');
    }
    return reactFlowModulePromise;
}

const createToolNode = (HandleComponent: ReactFlowModule['Handle'], PositionEnum: ReactFlowModule['Position']) =>
    ({ data, selected }: NodeProps) => {
        ...
        return (
            <div className={cn(
                ...
                <HandleComponent type="target" position={PositionEnum.Top} className="!bg-foreground-muted w-3 h-3" />
                ...
                <HandleComponent type="source" position={PositionEnum.Bottom} className="!bg-foreground-muted w-3 h-3" />
            </div>
        );
    };
...
type VisualPipelineCanvasProps = VisualPipelineProps & { module: ReactFlowModule };

const VisualPipelineCanvas = ({ workflowId, stepResults = {}, module }: VisualPipelineCanvasProps) => {
    const {
        ReactFlow,
        Background,
        BackgroundVariant,
    ...
    const ToolNode = useMemo(() => createToolNode(Handle, Position), [Handle, Position]);
    const nodeTypes = useMemo(() => ({ tool: ToolNode }), [ToolNode]);
...
export default function VisualPipelineDesignerWrapper(props: VisualPipelineProps) {
    const [module, setModule] = useState<ReactFlowModule | null>(null);
    ...
    useEffect(() => {
        ...
        loadReactFlowModule()
            .then((mod) => {
                if (mounted) setModule(mod);
            })
            .catch((err) => {
                console.error('Failed to load React Flow', err);
                if (mounted) setError('Failed to load visual editor');
            });
        return () => {
            mounted = false;
        };
    ...
    if (!module) {
        return <div className="p-8 text-center text-foreground-muted text-sm">Loading visual editor…</div>;
    }

    const { ReactFlowProvider } = module;

    return (
        <ReactFlowProvider>
            <VisualPipelineCanvas {...props} module={module} />
        </ReactFlowProvider>
    );
}
