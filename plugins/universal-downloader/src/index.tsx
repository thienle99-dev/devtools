...
// Mock API implementation if backend is missing (Demo Mode)
if (!(window as any).universalAPI) {
    const mockListeners: ((p: any) => void)[] = [];
    (window as any).universalAPI = {
        onProgress: (cb: any) => {
            mockListeners.push(cb);
            return () => { const i = mockListeners.indexOf(cb); if (i > -1) mockListeners.splice(i, 1); };
        },
        getHistory: async () => [],
        getSettings: async () => ({ downloadPath: '', defaultFormat: 'video', defaultQuality: '1080p' }),
        checkDiskSpace: async () => ({ available: 100 * 1024 * 1024 * 1024, total: 500 * 1024 * 1024 * 1024, warning: false }),
        getPendingCount: async () => 0,
        resumePending: async () => { },
        clearPending: async () => { },
        getInfo: async (url: string) => ({
            title: 'Demo Video (Mock Backend)',
            duration: 300,
            thumbnail: '',
            isPlaylist: false,
            platform: 'youtube', // valid platform type
            availableQualities: ['2160p', '1440p', '1080p', '720p', '480p'],
            playlistVideos: []
        }),
        download: async (opts: any) => {
            console.log('Mock download started:', opts);
            let p = 0;
            const interval = setInterval(() => {
                p += 2;
                mockListeners.forEach(cb => cb({
                    id: opts.id,
                    state: p >= 100 ? 'complete' : 'downloading',
                    percent: p,
                    downloaded: p * 1024 * 1024,
                    total: 100 * 1024 * 1024,
                    speed: 2 * 1024 * 1024,
                    eta: (100 - p) / 2,
        ...
        getQueue: async () => [],
        saveSettings: async () => { },
        chooseFolder: async () => '/Mock/Downloads',
        clearHistory: async () => { },
